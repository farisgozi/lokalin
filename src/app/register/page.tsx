...
import { setOAuthPendingRole } from "@/lib/auth/auth-service";
...
export default function RegisterPage() {
  ...
  async function onSubmit(e: FormEvent) {
    ...
    try {
      ...
      const message = err instanceof Error ? err.message : "Gagal register";
      setError(message);
    }
  }
  ...
  return (
    <section className="min-h-screen flex items-center justify-center px-4 py-12 bg-gradient-to-br from-orange-50 via-white to-pink-50 relative overflow-hidden">
      ...
      <div className="absolute top-1/2 left-1/2 w-72 h-72 bg-orange-100/40 rounded-full blur-3xl -translate-x-1/2 -translate-y-1/2" />
        ...
        <div className="bg-white/80 backdrop-blur-xl rounded-3xl border border-white/60 shadow-2xl shadow-orange-500/5 p-8 md:p-10">
          ...
          <div className="text-center mb-8">
            ...
            <p className="mt-2 text-sm text-gray-500">
              Bergabung dan dukung UMKM lokal Indonesia
            </p>
          </div>
          ...
          <div className="grid grid-cols-2 gap-3 mb-6">
            <button
              ...
              className={`relative flex flex-col items-center gap-2 p-4 rounded-2xl border-2 transition-all duration-300 ${role === "user"
                ? "border-orange-400 bg-orange-50 shadow-md shadow-orange-500/10"
                : "border-gray-200 bg-white hover:border-gray-300"
                }`}
            >
              <User
                className={`w-6 h-6 ${role === "user" ? "text-orange-500" : "text-gray-400"
                  }`}
              />
              <span
                className={`text-sm font-semibold ${role === "user" ? "text-orange-600" : "text-gray-600"
                  }`}
              >
                Pengguna
              </span>
              <span className="text-[10px] text-gray-400">Eksplor & Review</span>
              {role === "user" && (
                <motion.div
            ...
            <button
              ...
              className={`relative flex flex-col items-center gap-2 p-4 rounded-2xl border-2 transition-all duration-300 ${role === "owner"
                ? "border-orange-400 bg-orange-50 shadow-md shadow-orange-500/10"
                : "border-gray-200 bg-white hover:border-gray-300"
                }`}
            >
              <Store
                className={`w-6 h-6 ${role === "owner" ? "text-orange-500" : "text-gray-400"
                  }`}
              />
              <span
                className={`text-sm font-semibold ${role === "owner" ? "text-orange-600" : "text-gray-600"
                  }`}
              >
                Pemilik UMKM
              </span>
